...
manipulated NPV reliability using verbal prompts. That is, participants were
explicitly told whether or not NPV was reliable for a certain project.
...
replicated those of Experiment 1. For the numerical reliability condition, the
findings from a pilot experiment (detailed in Appendix \@ref(alignment-7)) were
used for the predictions. The pilot experiment found no significant differences
between numerical reliability conditions, which were similar to those used in
Experiment 3. Therefore, this result was also expected for the present numerical
reliability conditions. Appendix \@ref(alignment-8-appendix) shows a simulation
of these hypothesised effects. Therefore, as well as retesting
...
condition, Experiment 3 tested the following hypothesis:

```{hypothesis, four-way-alignment-8, name = "overall effect", echo = TRUE}
The alignment $\times$ reliability level $\times$ reliability type $\times$ NPV
interaction is significant such that the alignment, reliability level, and NPV
effects are significant for the verbal reliability condition but not for the
numerical reliability condition.
```
