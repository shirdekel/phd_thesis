...
Experiment 1 investigated the effects of anecdotal similarity and bias on
...
similarity moderates the anecdotal bias effect. Thus, the main question is
whether anecdotal bias will be greater when the anecdote is similar to the
target project compared with when it is dissimilar. The target project was
supported by the statistics but was inconsistent with the anecdotes. Further,
Experiment 1 only used negative anecdotes. Therefore, the experiment would show
evidence of anecdotal bias if participants assigned to the statistics only
condition allocated more money to the target project compared with those in the
anecdote & statistics condition. Thus, Experiment 1 tested the following
hypothesis:
...
statistics explanation) and those in the high-similarity anecdote only condition
will allocate capital equally to the target project.
```
